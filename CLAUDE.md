...
- **Radar di rosa: 6 assi tutti derivati da indici già calcolati, nessuna nuova fonte**:
  ...
  esterna, quindi l'asse vale onestamente 0. Sovrapposizione di tutte le rose sullo stesso
  grafico (Recharts `RadarChart`) invece di un radar per card: coerente con "grafici
  sovrapponibili", la funzionalità che la spec sez. 10 segnala come fondamentale, e permette il
  confronto diretto tra rose a colpo d'occhio durante l'asta.
