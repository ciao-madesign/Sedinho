...
| Dashboard (sez. 9) | 🚧 | Header collegato alla League reale (nome, partecipanti, budget, rosa) con CTA a `/setup` se non configurata. Delle 9 sezioni spec, 6 ora collegate a dati reali: Migliori occasioni, Sopravvalutati, Titolari, Rigoristi/piazzati (vedi §5 sui limiti di "nuovi") più **Trasferimenti** (Transfer Engine, sez. 6 — nuova rotta `GET /transfers/recent`) e **In crescita/in calo** (`PlayerListItem.fantasyAvgTrend`, delta di fantamedia Serie A tra le 2 stagioni più recenti con dato reale, da `GET /players`); restano placeholder onesti solo **Cambi di gerarchia** (richiede uno storico delle gerarchie nel tempo, oggi sovrascritte ad ogni import, mai tenute) e **Infortuni** (nessuna fonte collegata, tentativo Transfermarkt abbandonato — vedi §5). **Filtri implementati**: `DashboardFiltersBar` (`components/DashboardFilters.tsx`) — ruolo, squadra, fascia prezzo (quotazione), età, rischio (disponibilità), titolarità, tutti quelli richiesti dalla spec sez. 9 — filtra il pool di giocatori condiviso da tutte le sezioni (incluso il join per Trasferimenti) prima che ognuna estragga la propria top-5 (vedi §5 sulla scelta di una barra unica invece di controlli per pannello) |
| Sistema grafico (sez. 10) | 🚧 | Pagina dedicata `/confronti` ("Confronti" in nav, coerente con la direzione UX §9 "tab dedicate"), **due modalità**: **Giocatori** — selezione di fino a 4 giocatori (ricerca nome + filtro ruolo + filtro squadra + ordinamento, stesso pattern di `PlayersPage`, richiesto esplicitamente — non solo ricerca per nome), 6 grafici con Recharts: 4 **sovrapponibili** (la funzionalità che la spec segnala come "fondamentale") — fantamedia storica (linea), gol/assist ultima stagione (barre raggruppate), minuti ultima stagione (barre), quotazione ufficiale vs prezzo atteso (barre raggruppate), **fantamedia vs media ruolo/squadra** (barre raggruppate, medie calcolate su tutto il database non solo sui selezionati) — più uno **scatter quotazione/valore** indipendente dalla selezione (tutto il pool filtrabile per ruolo, punti colorati per ruolo, per scovare occasioni a colpo d'occhio); **Rose** — confronto tra le rose dei partecipanti a un'asta in corso (richiesto esplicitamente, non in spec), radar di rosa (sez. sotto) di tutti i partecipanti sovrapposto sullo stesso grafico, messaggio onesto se nessuna asta e' attiva (il radar di rosa esiste solo dentro un'asta). Scope Giocatori concordato con l'utente in due blocchi successivi (i 4 grafici sovrapponibili iniziali, poi "giocatore vs media" e "scatter valore/prezzo" scelti esplicitamente tra le opzioni proposte), non tutti i 9 tipi di grafico della spec: istogrammi, box plot, heatmap, distribuzioni, timeline restano ⬜ (radar e scatter ora fatti). "Prezzo reale" (spec: "prezzo stimato vs reale") non disponibile: nessuna asta con abbastanza dati storici, solo "prezzo atteso" dal motore di valutazione |
...
29. **Connettore notizie di mercato dell'ultima ora — da valutare prima del go-live, richiesto
    esplicitamente dall'utente**: durante un'asta live sarebbe utile sapere di scambi, nuovi
    acquisti, cessioni, infortuni dell'ultima ora per una squadra di Serie A, non catturabili
    dalle fonti già integrate (Fantacalcio.it/FSTATS/Fantacalciopedia si aggiornano con un ritmo
    diverso, non "ultima ora"). Non ancora progettato: da capire se via un connettore di
    scraping dedicato (stesso pattern di `import/connectors/`, ma innescato on-demand durante
    l'asta invece che da "Aggiorna Database"?) o via una ricerca web assistita da AI (BYOK,
    stesso pattern del Commento AI — l'utente fornirebbe la propria chiave, nessuna chiamata
    LLM lato server). Nessuna fonte specifica ancora scelta: da discutere con l'utente prima di
...
34. ~~Sistema grafico: 2 grafici aggiuntivi in Confronti~~ — fatti, scelti dall'utente tra 4
    ...
    `PlayerListItem.fantasyAvg`) e "scatter quotazione/valore" su tutto il pool. Vedi §5. Restano
