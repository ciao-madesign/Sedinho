...
19. L'utente ha offerto di fornire i dati delle scorse edizioni della propria lega (non Serie A
    ...
    il formato prima di progettare lo schema (probabile estensione del Transfer Engine, sez. 6,
    ancora ⬜), non scaffoldare una pipeline di import per dati ipotetici prima di vederli.
20. Prossimo passo concordato con l'utente: sistema grafico (sez. 10, Recharts installato,
    nessun grafico ancora costruito) — grafici sovrapponibili per confrontare giocatori/ruoli/
    squadre (funzionalità "fondamentale" secondo la spec).
